...
//    List<Task> findAllByIsArchived(Boolean isArchived);

//    @Query(value = "SELECT tk.id, tk.title, tk.description, tk.status, tk.priority, u.login, pr.title, tk.deadline, f.id FROM task_manager.tasks as tk inner join task_manager.users as u on tk.leader_id = u.id inner join task_manager.projects\n" +
